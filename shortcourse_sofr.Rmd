...
In this section we'll use the [NHANES_JSM_2019](./Data.html) to illustrate scalar-on-function regression (sofr).
...
We can now calcuale the functional regression parameter $\hat{\beta}(t)$ as a linear combination of fPCs.
...
In this example, we fit a logistic regression of Diabetes (binary outcome) on fPC1, fPC2, fPC3 scores adjusted for Age, BMI, and Gender.

```{r sofr_call_03, eval = TRUE}
## fit linear model with BMI as the outcome and PC1, PC2, PC3 as predictors
fit.2 <- glm(Diabetes ~ scale(Age) + scale(BMI) + Male + scale(fPC1.score) + scale(fPC2.score) + scale(fPC3.score), "binomial")
summary(fit.2)$call
summary(fit.2)$coefficients
```

Functional regression parameter $\beta(t)$ is estimated as a linear combination of the fPCs

```{r sofr_call_04, eval = TRUE}
beta <- fPC1*summary(fit.2)$coefficients[5,1] + fPC2*summary(fit.2)$coefficients[6,1] + fPC3*summary(fit.2)$coefficients[7,1]
